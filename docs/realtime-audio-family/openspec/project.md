# OpenSpec Project: Realtime Audio Family Standards

Status: reusable family OpenSpec baseline
Revision: 2026-05-23-family-standard

## Purpose

This OpenSpec package defines reusable requirements for realtime audio family apps. Product projects inherit these specs and add their own feature-specific specs.
...
Family specs live under `openspec/specs/`. Product-specific specs should be added under the same tree in the adopting project and should reference the family specs they inherit.

## Required project behavior

Every adopting project MUST:

- inherit `realtime-audio-core`;
- inherit `performance-gates`;
- define its backend adapter;
- define output routing behavior;
- define event ingress behavior;
- define telemetry behavior;
- define brownfield remediation if integrating with existing code;
- add stricter product-specific specs where needed.

## Change rule

No OpenSpec change may permit callback allocation, locks, waits, logging, I/O, UI, parser calls, device discovery, route discovery, dynamic graph mutation, or unbounded algorithms in callback-reachable code.
