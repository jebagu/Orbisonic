...
Control state exposes parameters and project state to the realtime core safely.

## Requirements

### Requirement: Latest-value controls

Coalescible controls SHOULD use latest-value atomic slots or immutable snapshots rather than ordered queues.

### Requirement: Prepared complex state

Complex state SHALL be prepared outside realtime and published as immutable snapshots.

### Requirement: Bounded smoothing

Realtime smoothing SHALL be bounded and preconfigured. Curve construction and validation SHALL happen outside realtime.
