...
Telemetry and metering observe audio but do not control audio timing.

## Requirements

### Requirement: Tiny callback snapshot

The callback MAY publish tiny fixed-size meter snapshots and counters.

### Requirement: Formatting outside realtime

Telemetry formatting, serialization, network emission, logging, and UI model construction SHALL happen outside realtime.

### Requirement: Lossy under overload

Telemetry SHALL drop, decimate, or coalesce under overload rather than delay audio.
