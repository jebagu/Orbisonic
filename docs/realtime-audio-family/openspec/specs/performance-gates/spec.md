...
Performance gates prove realtime safety.

## Requirements

### Requirement: Callback allocation gate

Callback allocation and deallocation counts SHALL be zero during stress tests.

### Requirement: Blocking gate

Callback blocking locks, waits, sleeps, joins, and condition waits SHALL be zero during stress tests.

### Requirement: Deadline gate

Deadline misses SHALL be zero during release stress tests.

### Requirement: Percentile reporting

The project SHALL report p50, p95, p99, and maximum callback duration under stress.

### Requirement: Active UI and telemetry

Stress tests SHALL run with UI, metering, diagnostics, and telemetry active when those features exist.
