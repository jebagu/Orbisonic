...
Output routing maps logical project audio to physical, host, file, or network outputs.

## Requirements

### Requirement: Explicit route map

The system SHALL use an explicit route map prepared before audio starts.

### Requirement: No silent fallback

The system SHALL NOT silently downmix, truncate, duplicate, reorder, or fallback unless a product-specific spec explicitly defines that behavior and the user is told before arming.

### Requirement: Callback uses prepared route snapshot

The callback SHALL only read precomputed route data. It SHALL NOT discover, validate, or repair routes.
