...
Brownfield projects adopt the family standard by inventorying callback reachability and remediating violations.

## Requirements

### Requirement: Callback inventory

A brownfield project SHALL identify all callback entry points and callback-reachable functions.

### Requirement: Violation inventory

The project SHALL inventory allocation, locks, waits, logging, I/O, UI, parser calls, route discovery, device discovery, and unbounded algorithms reachable from callbacks.

### Requirement: Remediation plan

Each violation SHALL have a remediation plan or accepted ADR exception before compliance is claimed.
