...
final class OrbisonicAudioCoordinator2Tests: XCTestCase {
    ...
    func testExistingUIFacadeMapsSourceModeSelectionIntoCoordinator() throws {
        ...
        let atmos = try facade.selectSource(.atmosDRP)
        XCTAssertEqual(atmos.path, .atmosDRPLivePcmCapture)
        XCTAssertEqual(facade.observeNowPlayingState().sourceMode, .atmosDRP)
        XCTAssertEqual(facade.observeNowPlayingState().preparedPath, .atmosDRPLivePcmCapture)
        XCTAssertEqual(facade.observeDiagnosticsState().snapshot?.sourceKind, .atmosDRP)
    }
